...
class Tax_Meta_Class {
   ...
   */
  public function __construct ( $meta_box, $taxonomy_name ) {
  ...
  public function insert_images() {
    ...
    foreach( $_POST['attachments'] as $attachment_id => $attachment ) {
      ...
      $li   .= "<a title='" . __( 'Delete this image',$this->taxonomy_name.'-meta') . "' class='at-delete-file' href='#' rel='{$nonce}|{$term_id}|{$id}|{$attachment_id}'><img src='" . $this->SelfPath. "/images/delete-16.png' alt='" . __( 'Delete',$this->taxonomy_name.'-meta') . "' /></a>";
  ...
  */
  public function wp_ajax_delete_image() {
    ...
    $ok = false;
    $remove_meta_only = apply_filters($this->taxonomy_name."_meta_class_delete_image",true);
    ...
    if ( $ok ){
      ...
      echo json_encode(array('message' => __( 'Cannot delete file. Something\'s wrong.',$this->taxonomy_name.'-meta')));
      die();
    }
  }
  ...
  public function show($term_id) {
    ...
    wp_nonce_field( basename(__FILE__), $this->taxonomy_name.'_meta_class_nonce' );
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $c = 0;
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        echo '" alt="'.__('Remove',$this->taxonomy_name.'-meta').'" title="'.__('Remove',$this->taxonomy_name.'-meta').'" id="remove-'.$field['id'].'"></div>';
        $c = $c + 1;
    ...
    echo '" alt="'.__('Add',$this->taxonomy_name.'-meta').'" title="'.__('Add',$this->taxonomy_name.'-meta').'" id="add-'.$field['id'].'"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_file( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      ...
      if ( !empty( $meta )  && count($meta) > 0 && !$this->is_array_empty($meta) ) {
        ...
        echo '<div style="margin-bottom: 10px"><strong>' . __('Uploaded files',$this->taxonomy_name.'-meta') . '</strong></div>';
        echo '<ol class="at-upload">';
        foreach ( $meta as $att ) {
          // if (wp_attachment_is_image($att)) continue; // what's image uploader for?
          echo "<li>" . wp_get_attachment_link( $att, '' , false, false, ' ' ) . " (<a class='at-delete-file' href='#' rel='{$nonce}||{$field['id']}|{$att}'>" . __( 'Delete',$this->taxonomy_name.'-meta' ) . "</a>)</li>";
        }
        echo '</ol>';
      }
    ...
    echo "<div class='at-file-upload-label'> \n
      <strong>" . __( 'Upload new files',$this->taxonomy_name.'-meta' ) . "</strong>\n
    </div>\n";
    echo "<div class='new-files'>\n
      <div class='file-input'>\n
        <input type='file' name='{$field['id']}[]' />\n
      </div><!-- End .file-input -->\n
      <a class='at-add-file button' href='#'>" . __( 'Add more files',$this->taxonomy_name.'-meta' ) . "</a>\n
      </div><!-- End .new-files -->\n";
    echo "</td>";
    $this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_color( $field, $meta ) {
    ...
    //  echo "<a href='#' class='at-color-select button' rel='{$field['id']}'>" . __( 'Select a color' ) . "</a>";
      echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . __( 'Select a color',$this->taxonomy_name.'-meta' ) . "'/>";
  ...
  public function save( $term_id ) {
    ...
    || ( ! check_admin_referer( basename( __FILE__ ), $this->taxonomy_name.'_meta_class_nonce') )    // Check nonce - Security
  ...
  public function get_tax_meta($term_id,$key,$multi = false){
    ...
    $m = get_option( $this->taxonomy_name.'_meta_'.$t_id);  
    if (isset($m[$key])){
      return $m[$key];
    }else{
      return '';
  ...
  public function delete_tax_meta($term_id,$key){
    $m = get_option( $this->taxonomy_name.'_meta_'.$term_id);
    if (isset($m[$key])){
      unset($m[$key]);
    }
    update_option($this->taxonomy_name.'_meta_'.$term_id,$m);
  }
  ...
  public function delete_taxonomy_metadata($term,$term_id) {
    delete_option( $this->taxonomy_name.'_meta_'.$term_id );
  }
